...
STARTUP_LOG     = Path(os.environ.get("LOCALAPPDATA", str(BASE_DIR))) / "Ultron Jarvis" / "startup.log"
...
def _ensure_desktop_shortcut() -> None:
    ...
    try:
        ...
        ps1_script = "\n".join([
            ...
            "$Shortcut.Description = 'Launch Ultron Jarvis'",
...
def _load_system_prompt() -> str:
    try:
        ...
        return (
            "You are Ultron Jarvis, a calm, direct, and professional AI assistant. "
...
def _speak_daily_briefing(ui=None) -> None:
    try:
        ...
        if ui:
            ...
            ui.write_log(f"Ultron Jarvis: {text}")
        speak_native(text)
...
def _gemini_text_reply(prompt: str) -> str:
    ...
    system_prompt = (
        "You are Ultron Jarvis, a concise, helpful desktop assistant. "
...
TOOL_DECLARATIONS = [
    ...
    },
    {
        ...
        "description": (
            ...
            "Ultron Jarvis automatically infers the best visual style from the topic, searches for a matching online template when available, "
        ...
        "parameters": {
            ...
            "properties": {
                ...
                "theme": {
                    "type": "STRING",
                    "description": "Optional presentation theme or visual direction such as neon, corporate, luxury, academic, sunset, or creative. If omitted, Ultron Jarvis infers the best style automatically."
                },
                "outline": {
                    "type": "STRING",
    ...
    },
    {
        ...
        "close the assistant, say goodbye, or stop Ultron Jarvis. "
...
class BrahmaLive:
        ...
        while True:
            ...
            try:
                if self._should_announce_idle():
                    ...
                    self.ui.write_log(f"Ultron Jarvis: {message}")
    ...
    def _on_text_command(self, text: str, source: str = "local"):
        ...
        if lower.startswith("email ") or lower.startswith("mail ") or "send email" in lower or "write email" in lower or "compose email" in lower:
            ...
            if not recipient:
                ...
                self.ui.write_log(f"Ultron Jarvis: {prompt}")
                self.speak(prompt)
                try:
        ...
        if _looks_like_screen_request(text):
            try:
                self.ui.update_task_workspace(
                    ...
                    output="Ultron Jarvis is inspecting the screen for what you asked about.",
                    percent=40,
                )
            except Exception:
                pass
    ...
    def _handle_smart_home_command(self, text: str, source: str = "local") -> bool:
        ...
        try:
            ...
            self.ui.write_log(f"Ultron Jarvis: {detail}")
            self.speak(detail)
    ...
    def _announce_attention(self, event: dict):
        ...
        self.ui.write_log(f"Ultron Jarvis: {msg}")
        if self.session and self._loop:
            self.speak(msg)
        else:
    ...
    def _on_meeting_update(self, payload: dict):
        ...
        if answer:
            self.ui.write_log(f"Ultron Jarvis: {answer}")
    ...
    def _handle_email_flow(self, text: str) -> bool:
        ...
        if self._email_step == 0:
            ...
            try:
                import urllib.parse
                import webbrowser
                subject = "Message from Ultron Jarvis"
                ...
                if "gmail" in app_lower or "chrome" in app_lower:
                    import urllib.parse
                    quoted_recipient = urllib.parse.quote(self._email_recipient)
                    quoted_subject = urllib.parse.quote("Message from Ultron Jarvis")
                    quoted_body = urllib.parse.quote(self._email_message)
    ...
    def _handle_attention_response(self, text: str) -> bool:
        ...
        if kind == "message":
            ...
            if self._attention_matches(lower, ("hear", "read", "what is it", "tell me", "show it", "open it")):
                ...
                self.ui.write_log(f"Ultron Jarvis: {preview}")
    ...
    def _fallback_reply(self, text: str, memory_ctx: str = ""):
        try:
            ...
            try:
                self.ui.update_task_workspace(
                    status="Thinking",
                    output="Ultron Jarvis is drafting a direct reply.",
                    percent=35,
                )
            except Exception:
                pass
            ...
            if not reply:
                reply = "I’m ready, sir."
            self.ui.write_log(f"Ultron Jarvis: {reply}")
            try:
    ...
    def _build_config(self) -> types.LiveConnectConfig:
        ...
        parts.append(
            "Wake-word mode: if the microphone is muted, still listen for the words 'Ultron Jarvis', 'hey', 'hi', and 'hello'. "
    ...
    async def _execute_tool(self, fc) -> types.FunctionResponse:
        ...
        if tool_voice:
            try:
                self.ui.write_log(f"Ultron Jarvis: {tool_voice}")
            except Exception:
                pass
            try:
                ...
                async for response in self.session.receive():
                    ...
                    if response.server_content:
                        ...
                        if sc.turn_complete:
                            ...
                            if full_out:
                                self.ui.write_log(f"Ultron Jarvis: {full_out}")
                            out_buf = []
                ...
                try:
                    async with asyncio.TaskGroup() as tg:
                        ...
                        try:
                            ...
                            pass
                        self.ui.set_state("LISTENING")
                        self.ui.write_log("SYS: Ultron Jarvis online.")
...
def main():
    ...
    if DashboardServer is not None and not dashboard_enabled:
        ...
        try:
            ui.write_log("SYS: Mobile Connect is already running in another Ultron Jarvis instance.")
        except Exception:
            pass
