...
try:
    from core.paths import get_config_dir, get_base_dir
    _PATHS_OK = True
except Exception:
    _PATHS_OK = False

def _get_base_dir_fallback() -> Path:
    """Fallback base-dir when core.paths is unavailable."""
    import sys
...
class IdentityService:
    def __init__(self):
        if _PATHS_OK:
            new_path = get_config_dir() / "identity.json"
            legacy = get_base_dir() / "config" / "identity.json"
            # Automatic migration: copy legacy config to new location if needed
            if not new_path.exists() and legacy.exists():
                new_path.parent.mkdir(parents=True, exist_ok=True)
                shutil.copy2(legacy, new_path)
            self.config_file = new_path
        else:
            self.config_file = _get_base_dir_fallback() / "config" / "identity.json"
        self.data: Dict[str, Any] = {
            ...
            "assistant": {
                "name": "Ultron Jarvis",
                "application_name": "Ultron Jarvis (Hidayat AI)",
                "title": "Ultron Jarvis - Hidayat AI"
            },
            "behavior": {
                "mode": "professional",
                "proactive": True,
    ...
    def get_assistant_name(self) -> str:
        return self.data["assistant"].get("name", "Ultron Jarvis")
    ...
    def get_application_name(self) -> str:
        return self.data["assistant"].get("application_name", "Ultron Jarvis (Hidayat AI)")
    ...
    def get_assistant_title(self) -> str:
        return self.data["assistant"].get("title", "Ultron Jarvis - Hidayat AI")
