...
def _fallback_spec(parameters: dict[str, Any]) -> dict[str, Any]:
    ...
    site_name = _sanitize_text(
        str(parameters.get("site_name") or parameters.get("title") or "Hidayat AI Project"),
        "Hidayat AI Project",
    )
    ...
    pages = [
        {
            ...
            "hero": {
                "eyebrow": "Built with Ultron Jarvis",
                "title": site_name,
...
Hard rules:
- Do not mention Ultron Jarvis, Codex, or any assistant branding in the site content.
...
def _normalize_page(page: dict[str, Any], site_name: str, brief: str, idx: int) -> dict[str, Any]:
    ...
    hero_data = {
        "eyebrow": _sanitize_text(str(hero.get("eyebrow") or ("Built with Ultron Jarvis" if idx == 0 else "Project Page")), "Built with Ultron Jarvis"),
    ...
    return f"""
      <div class="workspace-shell">
        <aside class="workspace-sidebar">
          <div class="sidebar-brand">Ultron Jarvis Workspace</div>
...
<body id="top">
  ...
  <header class="topbar">
    <div class="brand">
      ...
      <div>
        ...
        <span>{_html(tagline or "Generated with Ultron Jarvis")}</span>
      </div>
    </div>
  ...
  <footer class="footer reveal">
    <div>
      <strong>{_html(spec.get("site_name") or "Website")}</strong>
      <p>{_html(tagline or "Generated with Ultron Jarvis")}</p>
    </div>
...
<body id="top">
  ...
  <main class="layout-{_html(layout_mode)}">
    <section class="hero hero--{_html(layout_mode)} reveal">
      <div class="hero-copy">
        <p class="eyebrow">{_html(hero.get("eyebrow") or "Built with Ultron Jarvis")}</p>
        <h1>{_html(hero.get("title") or title)}</h1>
        <p class="lead">{_html(hero.get("subtitle") or tagline)}</p>
        <div class="hero-actions">
...
Generated by Ultron Jarvis (Hidayat AI).
...
def _open_vscode(project_dir: Path) -> bool:
    ...
    candidates = [
        "code",
        "code.cmd",
        os.path.join(local_appdata, "Programs", "Microsoft VS Code", "bin", "code.cmd") if local_appdata else None,
