...
class RemoteKeyOverlay(QWidget):
    ...
    def __init__(self, url: str, key: str, auto: str, manual: str, parent=None):
        ...
        subtitle = QLabel("Scan the QR code with your phone to remotely control Ultron Jarvis.")
    ...
    def mark_connected(self) -> None:
        ...
        self._timer_lbl.setText("Phone connected. Ultron Jarvis remote is ready.")
...
def _hidden_launch_args(*extra_args: str) -> list[str]:
    ...
    # Prefer pythonw from virtual environment, then system pythonw, then current python
    venv_pythonw = BASE_DIR / ".venv" / "Scripts" / "pythonw.exe"
    if venv_pythonw.exists():
        return [str(venv_pythonw), str(main_py), *extra_args]
    # Try to find system pythonw
    system_pythonw = shutil.which("pythonw.exe")
    if system_pythonw:
        return [system_pythonw, str(main_py), *extra_args]
...
def _startup_run_value() -> str:
    ...
    system_pythonw = shutil.which("pythonw.exe")
    if system_pythonw:
        return f'{_quote_cmd_arg(system_pythonw)} {_quote_cmd_arg(str(main_py))} --startup'
...
class TaskCard(QFrame):
    def __init__(self, parent=None):
        ...
        self._plan_lbl = QLabel("Plan: Ultron Jarvis will generate a task plan after you send a command.")
    ...
    def set_task(self, title: str, desc: str, percent: int):
        ...
        self._plan_lbl.setText("Plan: Ultron Jarvis will generate a task plan after you send a command.")
...
class ChatBubble(QFrame):
    def __init__(self, role: str, name: str, text: str, stamp: str, attachments: list[dict] | None = None, parent=None, animate: bool = False):
        ...
        if role == "assistant":
            ...
            name_lbl = QLabel(name or "Ultron Jarvis")
            name_lbl.setFont(QFont("Segoe UI", 9, QFont.Weight.Bold))
            name_lbl.setStyleSheet("color: #ffffff; background: transparent;")
...
class ConversationFeed(QScrollArea):
    ...
    def _ensure_empty_widget(self):
        ...
        lay.setSpacing(10)
        title = QLabel("Try asking Ultron Jarvis")
    ...
    def load_messages(self, messages: list[dict[str, Any]]):
        ...
        for msg in messages:
            ...
            name = {
                "user": "You",
                "assistant": "Ultron Jarvis",
                "system": "System",
                "file": "Files",
            }.get(role, "Ultron Jarvis")
...
class WorkspaceSidebar(QWidget):
    ...
    def record_chat_event(self, event: object):
        ...
        if role == "user":
            ...
            self._active_conversation_id = convo_id
            self._feed.add_message("assistant", "Ultron Jarvis", text, _fmt_time_stamp(stamp), attachments=attachments, animate=True)
            self._hide_memory_banner()
        elif role == "system":
...
class InlineChatWorkspace(QFrame):
    ...
    def _build_chat_tab(self) -> QWidget:
        ...
        self._footer_status = QLabel("Ultron Jarvis is listening...")
    ...
    def record_chat_event(self, event: object):
        ...
        if role == "user":
            ...
            self._feed.add_message("assistant", "Ultron Jarvis", text, stamp, attachments=attachments)
            self._hide_memories()
        elif role == "system":
...
class LauncherControlPanel(QDialog):
    def __init__(self, *, startup_workspace: bool = False, on_open=None, on_close=None,
        ...
        self._restart_btn = mk_btn("Restart Ultron Jarvis")
        self._quit_btn = mk_btn("Quit Ultron Jarvis")
    ...
    def _hide_icon_confirm(self):
        ...
        flay.setSpacing(10)
        lbl = QLabel("Hide Ultron Jarvis icon?")
...
class FileDropZone(QWidget):
    ...
    def _browse(self):
        path, _ = QFileDialog.getOpenFileName(
            self, "Select a file for Ultron Jarvis", str(Path.home()),
            "All Files (*.*);;"
...
class SetupOverlay(QWidget):
    ...
    def _build_stage3(self):
        ...
        self._intro_lines = []
        for txt in ["Identity confirmed.", "Hello.", "I'm Ultron Jarvis.", "Ready whenever you are."]:
            lbl = QLabel(txt)
            lbl.setAlignment(Qt.AlignmentFlag.AlignCenter)
            if txt == "I'm Ultron Jarvis.":
        ...
        self._launch_btn = QPushButton("Launch Ultron Jarvis →")
    ...
    def _show_intro_final(self):
        """Show the Ultron Jarvis intro sequence."""
        page = self._stack.widget(2)
        lay = page.layout()
...
class CommandBar(QWidget):
    ...
    def __init__(self, parent=None):
        ...
        self._input.setPlaceholderText("Tell Ultron Jarvis what to do...")
...
class DeveloperModeDialog(QDialog):
    def __init__(self, parent=None, settings: dict | None = None):
        ...
        desc = QLabel("Pick a workspace folder Ultron Jarvis should use when building websites or other workspace-based tasks.")
...
class MeetingOverlay(QWidget):
    ...
    def __init__(self, parent=None):
        ...
        self._answer = QLabel("Ultron Jarvis will show the live answer here.")
...
class FloatingLauncher(QWidget):
    ...
    def paintEvent(self, event):
        ...
        # ── 6. Hindi Ultron Jarvis text "ब्रह्मा इको" ──
    ...
    def _apply_state_style(self):
        self.setToolTip(f"Ultron Jarvis\n{self._status_line}")
        self.update()
...
class MainWindow(QMainWindow):
    ...
    def __init__(self, face_path: str):
        ...
        self.setWindowTitle("Ultron Jarvis")
    ...
    def _startup_enabled(self) -> bool:
        ...
        try:
            with winreg.OpenKey(
                ...
                try:
                    value, _ = winreg.QueryValueEx(key, "Ultron Jarvis")
                    run_value = _startup_run_value()
                    if value != run_value:
                        winreg.SetValueEx(key, "Ultron Jarvis", 0, winreg.REG_SZ, run_value)
                    return bool(value)
        ...
        try:
            with winreg.CreateKey(winreg.HKEY_CURRENT_USER, _startup_registry_key()) as key:
                if enabled:
                    ...
                    try:
                        winreg.DeleteValue(key, "Ultron Jarvis")
                    except FileNotFoundError:
                        pass
            return True
        except Exception as e:
    ...
    def _tick_clock(self):
        ...
        if hasattr(self, "_core_status_lbl") and self._core_status_lbl is not None:
            self._core_status_lbl.setText("Ultron Jarvis is ready. Gemini 2.5 Flash · OpenRouter · Voice Connected · Memory Enabled")
    ...
    def _build_footer(self) -> QWidget:
        ...
        lay.addStretch()
        lay.addWidget(_fl("Suryaansh Tiwari  Â·  Ultron Jarvis  Â·  Open Source"))
        lay.addStretch()
    ...
    def _browse_attachment(self):
        path, _ = QFileDialog.getOpenFileName(
            self, "Attach a file to Ultron Jarvis", str(Path.home()),
            "All Files (*.*);;"
            "Images (*.jpg *.jpeg *.png *.gif *.webp *.bmp *.svg);;"
            "Documents (*.pdf *.docx *.txt *.md *.pptx);;"
            "Data (*.csv *.xlsx *.json *.xml);;"
    ...
    def notify_phone_connected(self):
        ...
        self._log_sig.emit("SYS: Phone connected to Ultron Jarvis remote.")
    ...
    def _apply_state(self, state: str):
        ...
        if hasattr(self, "_task_card"):
            if state == "THINKING":
                self._task_card.set_task("Working on it...", "Ultron Jarvis is processing your request.", 72)
            elif state == "SPEAKING":
                self._task_card.set_task("Responding...", "Ultron Jarvis is speaking now.", 100)
            elif state == "MUTED":
                self._task_card.set_task("Microphone muted", "Voice input is paused.", 0)
            else:
                self._task_card.set_task("Ready", "Ultron Jarvis is idle and ready.", 0)
        if hasattr(self, "_result_card"):
            if state == "THINKING":
    ...
    def _on_setup_done(self, key: str, or_key: str, os_name: str):
        try:
            ...
            if self._overlay:
                ...
                self._overlay = None
            self._apply_state("LISTENING")
            self._log.append_log(f"SYS: Initialised. OS={os_name.upper()}. Ultron Jarvis online.")
        except Exception as e:
    ...
    def _build_left_panel_modern(self) -> QWidget:
        ...
        self._nav_items["home"] = NavItem("Ultron Jarvis Home", active=False, letter="H")
    ...
    def _build_command_row(self) -> QHBoxLayout:
        ...
        self._input = QLineEdit()
        self._input.setPlaceholderText("Ask Ultron Jarvis anything...")
...
class SystemConnectivitySidebar(QFrame):
    def __init__(self, controller=None, parent=None):
        ...
        self._mk_quick_action("Γå╗ Restart Ultron Jarvis", QStyle.StandardPixmap.SP_BrowserReload, self._restart)
...
class SettingsHubPage(QWidget):
    def __init__(self, navigation_callback, parent=None):
        ...
        subtitle = QLabel("Select a section below to configure your Ultron Jarvis environment.")
        ...
        cards_data = [
            ("Ultron Jarvis Home", "Configure smart home integrations", "🏠", 1),
...
class SystemConnectivityPage(QWidget):
    ...
    def _build_left_column(self):
        ...
        # Mobile connect
        mobile = self._card("Mobile Connect", "Connect your phone and control Ultron Jarvis remotely.")
        ml = mobile.layout()
        ...
        # Startup
        startup = self._card("Startup", "Use Ultron Jarvis with Windows startup preferences.")
        sl = startup.layout()
        self._startup_launch_btn = self._mk_toggle("Launch Ultron Jarvis when Windows starts", bool(self._load_app_settings().get("show_workspace_on_startup", False)), self._toggle_startup_from_page)
        ...
        # Data Location
        dataloc = self._card("Data Location", "Choose where Ultron Jarvis stores its data files.")
        dl = dataloc.layout()
        self._dataloc_path_label = QLabel(self._resolve_data_dir_str())
        self._dataloc_path_label.setWordWrap(True)
        self._dataloc_path_label.setStyleSheet(f"color: {C.TEXT_MED};")
        dl.addWidget(self._dataloc_path_label)
        btn_row_dl = QHBoxLayout()
        btn_row_dl.setSpacing(12)
        self._dataloc_change_btn = QPushButton("Change...")
        self._dataloc_change_btn.clicked.connect(self._change_data_location)
        self._dataloc_portable_btn = QPushButton("Use Portable Mode")
        self._dataloc_portable_btn.setCheckable(True)
        self._dataloc_portable_btn.setChecked(bool(self._load_app_settings().get("use_portable_mode", False)))
        self._dataloc_portable_btn.setCursor(Qt.CursorShape.PointingHandCursor)
        self._dataloc_portable_btn.clicked.connect(self._toggle_portable_mode)
        self._dataloc_reset_btn = QPushButton("Reset to Default")
        self._dataloc_reset_btn.clicked.connect(self._reset_data_location)
        btn_row_dl.addWidget(self._dataloc_change_btn, 1)
        btn_row_dl.addWidget(self._dataloc_portable_btn, 1)
        btn_row_dl.addWidget(self._dataloc_reset_btn, 1)
        dl.addLayout(btn_row_dl)
        self._dataloc_note = QLabel("")
        self._dataloc_note.setWordWrap(True)
        self._dataloc_note.setStyleSheet(f"color: {C.TEXT_DIM}; font-style: italic;")
        dl.addWidget(self._dataloc_note)
        lay.addWidget(dataloc)

        # Shortcuts & Pinning
        shortcuts = self._card("Shortcuts & Pinning", "Create shortcuts and pin Ultron Jarvis to your Windows system.")
        ...
        # App Theme
        theme_card = self._card("App Theme", "Select the primary color theme for Ultron Jarvis.")
        ...
        # Discord bot
        discord = self._card("Discord Bot", "Mirror Ultron Jarvis between the app and your server.")
        dl = discord.layout()
        ...
        about = self._card("About Ultron Jarvis", "Ultron Jarvis information only.")
        ab = about.layout()
    ...
    def _build_quick_actions_box(self):
        ...
        lay = box.layout()
        actions = [
            ("Restart Ultron Jarvis", QStyle.StandardPixmap.SP_BrowserReload, self._restart_app),
    ...
    def _resolve_data_dir_str(self) -> str:
        """Resolve and return the current data directory as a display string."""
        try:
            from core import paths as _paths
            return str(_paths.get_data_dir())
        except Exception:
            return "(unable to resolve)"

    def _change_data_location(self):
        """Open a directory picker and set an explicit data_directory path."""
        path = QFileDialog.getExistingDirectory(self, "Select Data Directory")
        if not path:
            return
        settings = self._load_app_settings()
        settings["data_directory"] = path
        settings["use_portable_mode"] = False
        try:
            os.makedirs(APP_SETTINGS_FILE.parent, exist_ok=True)
            with open(APP_SETTINGS_FILE, "w", encoding="utf-8") as f:
                json.dump(settings, f, indent=4)
        except Exception as exc:
            print(f"Error saving data location: {exc}")
        self._dataloc_path_label.setText(path)
        self._dataloc_portable_btn.setChecked(False)
        self._dataloc_note.setText("Restart required to apply changes.")
        if self._ctrl() and hasattr(self._ctrl(), "write_log"):
            self._ctrl().write_log(f"SYS: Data directory changed to {path}.")

    def _toggle_portable_mode(self):
        """Enable or disable portable mode (data stored next to executable)."""
        checked = self._dataloc_portable_btn.isChecked()
        settings = self._load_app_settings()
        settings["use_portable_mode"] = bool(checked)
        if checked:
        ...
        try:
            ...
            print(f"Error saving portable mode: {exc}")
        self._dataloc_path_label.setText(self._resolve_data_dir_str())
        self._dataloc_note.setText("Restart required to apply changes.")
        if self._ctrl() and hasattr(self._ctrl(), "write_log"):
            mode = "enabled" if checked else "disabled"
            self._ctrl().write_log(f"SYS: Portable mode {mode}.")

    def _reset_data_location(self):
        """Clear data_directory and use_portable_mode, revert to OS-default path."""
        settings = self._load_app_settings()
        settings.pop("data_directory", None)
        settings.pop("use_portable_mode", None)
        try:
            os.makedirs(APP_SETTINGS_FILE.parent, exist_ok=True)
            with open(APP_SETTINGS_FILE, "w", encoding="utf-8") as f:
                json.dump(settings, f, indent=4)
        except Exception as exc:
            print(f"Error resetting data location: {exc}")
        self._dataloc_path_label.setText(self._resolve_data_dir_str())
        self._dataloc_portable_btn.setChecked(False)
        self._dataloc_note.setText("Restart required to apply changes.")
        if self._ctrl() and hasattr(self._ctrl(), "write_log"):
            self._ctrl().write_log("SYS: Data location reset to default.")
    ...
    def _test_discord_from_page(self):
        ...
        if self._ctrl() and hasattr(self._ctrl(), "_win"):
            ...
            self._discord_msg.setText("Connected as Ultron Jarvis#9649" if self._discord_token.text().strip() else "Bot Offline")
    ...
    def refresh(self):
        ...
        if enabled and token:
            ...
            self._discord_msg.setText("Connected as Ultron Jarvis#9649")
        elif token:
    ...
    def _create_desktop_shortcut_logic(self):
        try:
            ...
            desktop_dir.mkdir(parents=True, exist_ok=True)
            shortcut_path = desktop_dir / "Ultron Jarvis - Premium.lnk"
            ...
            ps1_script = "\n".join([
                ...
                "$Shortcut.Description = 'Launch Ultron Jarvis - Premium'",
    ...
    def _pin_app_to_taskbar_logic(self):
        try:
            ...
            if res.returncode == 0:
                return True, "Ultron Jarvis has been pinned to your Taskbar!"
            else:
                return False, "Windows restricts programmatic taskbar pinning. Please right-click the 'Ultron Jarvis - Premium.lnk' shortcut on your Desktop and select 'Pin to taskbar', or drag it directly onto your taskbar."
        except Exception as e:
...
class SmartDevicesSection(QFrame):
    def __init__(self, controller=None, parent=None):
        ...
        empty_btn = QPushButton("Open Ultron Jarvis Home")
...
class BrahmaUI:
    def __init__(self, face_path: str, size=None, *, show_immediately: bool = True):
        ...
        self._app.setApplicationDisplayName("Ultron Jarvis")
        ...
        self._tray.setToolTip("Ultron Jarvis")
...
class BrahmaConnectDevicesPage(QFrame):
    def __init__(self, parent=None):
        ...
        self._subtitle = QLabel("Everything connected to Ultron Jarvis.")
