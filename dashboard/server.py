"""
dashboard/server.py — Ultron Jarvis (Hidayat AI) Local HTTP Dashboard
...
def _make_uploads_dir() -> Path:
    ...
    for candidate in [
        Path.home() / "Downloads" / "Ultron Jarvis Uploads",
        Path.home() / "Documents" / "Ultron Jarvis Uploads",
        BASE_DIR / "uploads",
    ]:
        try:
...
def _ensure_network_access(port: int) -> None:
    ...
    if sys.platform == "win32":
        ...
        port_rule = f"Ultron Jarvis Dashboard Port {port}"
        prog_rule  = "Ultron Jarvis Dashboard Python"
        ...
        try:
            ...
            if int(ret) > 32:
                ...
                print("[Dashboard] Phone connections may fail until Ultron Jarvis is run as Administrator.")
...
def _ensure_ssl_certs() -> bool:
    ...
    try:
        ...
        subject = issuer = x509.Name([
            x509.NameAttribute(NameOID.COMMON_NAME, "Ultron Jarvis Local Remote"),
        ])
        alt_names = [
...
<p>Press <strong style="color:#dde3ed">Mobile Connect</strong> in Ultron Jarvis to get a new QR code.</p>
</div></body></html>""")
...
<p>Connecting to Ultron Jarvis…</p>
</body></html>""")
    ...
    async def serve(self) -> None:
        ...
        print("[Dashboard] Press 'Mobile Connect' in Ultron Jarvis UI to get the QR code.")
        await uvicorn.Server(cfg).serve()
